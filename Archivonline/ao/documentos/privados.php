...
$query_Recordset1 = "SELECT * FROM documentos WHERE documentos.Categoria = 2";
...
mysql_select_db($database_archivo, $archivo);
$query_Recordset1 = sprintf("SELECT * FROM documentos WHERE documentos.Categoria = 2 AND documentos.Uplauder = %s", GetSQLValueString($varDato_Recordset1, "text"));
$Recordset1 = mysql_query($query_Recordset1, $archivo) or die(mysql_error());
$row_Recordset1 = mysql_fetch_assoc($Recordset1);
...
<!-- InstanceEndEditable -->
  ...
  <div class="content">
    ...
    <div class="inicio" style="height:auto; margin-bottom:0px; width:965px; float:left; padding:10px; padding-top:0px; padding-left:0px">
     <?php  if (isset($row_Recordset1['Uplauder'])) { ?>
       <?php do { ?> 
	...
	padding-left: 0px;">&nbsp;</p>
          <p style="font-size:11px;  padding-right: 0px;
	padding-left: 0px; float:right"><?php if ($row_Recordset1['Verificacion'] == 2 ) {?>Prof. <?php echo $row_Recordset1['Profesor'];  ?><?php } else { ?><?php echo $row_Recordset1['Profesor']; } ?><?php if ($row_Recordset1['Verificacion'] > 1) { ?>
              <img src="../../img/agregados/1410926033_678134-sign-check-256.png" width="9" height="9" title="Verificado" /> 
            <?php } ?></p>
            
        </div>
    </div></a><?php } while ($row_Recordset1 = mysql_fetch_assoc($Recordset1)); }  else { ?><div style="width:auto; alignment-adjust:central; padding-top:20px; color:#CC3333; margin-top:10px; margin-left:10px" class="advertencia">
      <h1>No tienes documentos privados</h1></div><?php } ?>
    ...
    <div style="float:left ; margin-right:740px;" class="paginacion"><a href="<?php printf("%s?pageNum_Recordset1=%d%s", $currentPage, max(0, $pageNum_Recordset1 - 1), $queryString_Recordset1); ?>">Anterior</a></div>
    
    
    <div style="float:left" class="paginacion"><a href="<?php printf("%s?pageNum_Recordset1=%d%s", $currentPage, min($totalPages_Recordset1, $pageNum_Recordset1 + 1), $queryString_Recordset1); ?>">Siguiente</a></div>
    </div>
  </div>
